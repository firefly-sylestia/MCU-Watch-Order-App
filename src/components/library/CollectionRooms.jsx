...
export default function CollectionRooms({
  ...
  return (
    <section
      className="collection-rooms archive-shelf"
    ...
    >
      ...
      <div className="collection-rooms__grid">
        {collections.map((collection) => {
          ...
          const watched = roomItems.filter(
            (item) => item.status === "watched",
          ).length;
          const pct = roomItems.length
            ? Math.round((watched / roomItems.length) * 100)
            : 0;
          ...
          return (
            ...
            >
              <span className="collection-room-card__icon">
                {collection.icon}
              </span>
              <span className="collection-room-card__content">
                ...
                <span className="collection-room-card__meta-row">
                  <span
                    className="collection-room-card__progress"
                    aria-label={`${pct}% watched`}
                  >
                    <b>{pct}%</b>
                    <span
                      className="collection-room-card__progress-pill"
                      aria-hidden="true"
                    >
                      <i style={{ width: `${pct}%` }} />
                    </span>
                  </span>
                  <span className="collection-room-card__stats">
                    {roomItems.length} items · {watched} watched
                  </span>
                </span>
              </span>
              <span
                className="collection-room-card__posters"
                aria-hidden="true"
              >
