...
export default function CinematicLibraryOS(props) {
  ...
  const activeView = browseMode === 'search' ? 'search' : browseMode === 'phase' ? 'collections' : browseMode || 'home';
  ...
  return (
    <div className="cinematic-library-os">
      ...
      {mobileMenu && <div className="archive-sheet" role="dialog" aria-modal="true" aria-label="More archive actions"><div className="archive-sheet__panel"><button className="archive-sheet__close" type="button" onClick={() => setMobileMenu(false)}><X size={16} /> Close</button><button onClick={() => { openSettings(); setMobileMenu(false); }}>Settings</button><button onClick={() => { openAnalytics(); setMobileMenu(false); }}>Progress Analytics</button><button onClick={() => { navigate('collections'); setMobileMenu(false); }}>Collections</button><button onClick={() => { navigate('search'); setMobileMenu(false); }}>Search Catalog</button></div></div>}
