...
const searchRoutePath = (query) => {
  ...
  return clean ? `/search/${encodeURIComponent(slugifyPosterName(clean))}` : '/search';
};
...
export default function MCUViewer() {
  ...
  const openSearchMode = useCallback((query = '') => {
    setDetailItem(null);
    setSettingsOpen(false);
    setAnalyticsOpen(false);
    setTrailerOpen(false);
  ...
  const openSeriesMode = useCallback((query = '') => {
    setDetailItem(null);
    setSettingsOpen(false);
    setAnalyticsOpen(false);
    setTrailerOpen(false);
  ...
  const openPhaseMode = useCallback((phaseId = 0) => {
    setDetailItem(null);
    setSettingsOpen(false);
    setAnalyticsOpen(false);
    setTrailerOpen(false);
    ...
    setBrowseMode('phase');
    setTypeFilter(null);
    setActivePhase(Number.isFinite(Number(phaseId)) ? Number(phaseId) : 0);
  }, []);
  ...
  const applyUrlRoute = useCallback((path = window.location.pathname) => {
    ...
    if (primary === 'search') {
      ...
      const queryFromPath = parts.slice(1).join(' ');
      const queryFromUrl = urlParams.get('q') || urlParams.get('query') || queryFromPath;
      setDetailItem(null);
      setSettingsOpen(false);
      setAnalyticsOpen(false);
      setBrowseMode('search');
      ...
      setTypeFilter(null);
      setSearchScope(UI_STATE_DEFAULTS.searchScope);
      setSearch(decodeURIComponent(queryFromUrl || '').replace(/-/g, ' ').trim());
      return;
    }

    if (primary === 'series') {
      const requestedSeries = parts.slice(1).join(' ');
      setDetailItem(null);
      setSettingsOpen(false);
      setAnalyticsOpen(false);
      setBrowseMode('search');
      ...
      setTypeFilter('series');
      setSearchScope(UI_STATE_DEFAULTS.searchScope);
      setSearch(decodeURIComponent(requestedSeries || '').replace(/-/g, ' ').trim());
      return;
    }
  ...
  const canonicalRoute = useMemo(() => {
    ...
    if (browseMode === 'search') {
      if (typeFilter === 'series') return search.trim() ? `/series/${encodeURIComponent(slugifyPosterName(search))}` : '/series';
      return searchRoutePath(search);
    }
  ...
  useEffect(() => {
    ...
    const method = currentPath === '/' || canonicalRoute === ROUTE_FALLBACK || browseMode === 'search' ? 'replaceState' : 'pushState';
    ...
    <div data-scaffold={Boolean(sectionScaffold)} data-theme={normalizeAppearanceMode(appearanceMode)} data-universe={universe === 'dc' ? 'dc' : 'marvel'} style={{ ...cssThemeVars, '--row-gap': densityMode === 'compact' ? '8px' : '12px', '--row-pad': densityMode === 'compact' ? '11px 10px 11px 8px' : '16px 16px 16px 12px', '--row-min-h': densityMode === 'compact' ? '72px' : '86px', '--text-scale': 1, '--ui-scale': effectiveUiScale, minHeight: '100dvh', backgroundColor: 'var(--app-bg-base)', backgroundImage: appTexture !== 'none' ? `${appTexture}, ${appThemeBg}` : appThemeBg, backgroundSize: appTexture !== 'none' ? '6px 6px, auto' : 'auto', color: 'var(--theme-text)', fontFamily: 'var(--font-marvel-body)', fontSize: '16px', zoom: effectiveUiScale, display: 'flex', flexDirection: 'column', overflowX: 'hidden', overflowY: 'visible', touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', transition: 'background 260ms var(--ease-out), color 180ms var(--ease-out)' }} className={`theme-switch ${universe === 'dc' ? 'dc-universe' : 'mcu-universe'}${performanceMode || browseMode === 'phase' ? ' performance-mode' : ''}${themeTransitioning ? ' theme-is-transitioning' : ''}${uiBuildCacheEnabled ? ' ui-build-cache-on' : ''}${overlayActive ? ' overlay-open' : ''}${browseMode === 'phase' ? ' phase-list-mode' : ''}`} data-color-mode={darkMode ? 'dark' : 'light'}>
      ...
      <NavigationShell controlsHidden={analyticsOpen || detailItem || sidebarOpen || settingsOpen} ref={sidebarRef} open={sidebarOpen} darkMode={darkMode} performanceMode={performanceMode} pillBorder={T.pillBorder} surfaceBorder={T.surfaceBorder} onToggle={toggleSidebarPanel} onClose={closeSidebar} onDismissBackdrop={suppressNextDocumentClick} onOpenSettings={toggleSettingsPanel}>
        <div className="sidebar-redesign">
          ...
          <section className="sidebar-panel sidebar-panel--links">
            <div className="sidebar-section-title">{tUniverse('Deep Links')}</div>
            <div className="sidebar-route-grid">
              {[
                { href: '/home', label: 'Home', active: browseMode === 'home' && !settingsOpen && !analyticsOpen, onClick: navigateHome },
                { href: searchRoutePath(search), label: 'Search', active: browseMode === 'search' && typeFilter !== 'series', onClick: () => openSearchMode(search) },
                { href: activePhase ? `/phase/${activePhase}` : '/phase', label: 'Phases', active: browseMode === 'phase', onClick: () => openPhaseMode(activePhase) },
                { href: search.trim() && typeFilter === 'series' ? `/series/${slugifyPosterName(search)}` : '/series', label: 'Series', active: browseMode === 'search' && typeFilter === 'series', onClick: () => openSeriesMode(typeFilter === 'series' ? search : '') },
              ].map(link => (
                <a
                  key={link.href + link.label}
                  className={`sidebar-route-link ${link.active ? 'is-active' : ''}`}
                  href={link.href}
                  onClick={(event) => {
                    event.preventDefault();
                    link.onClick();
          ...
          <div className="search-page-panel" style={{ border: `1px solid ${T.filterBorder}`, borderRadius: 18, padding: 14, background: 'color-mix(in srgb, var(--theme-surface) 84%, transparent)', boxShadow: '0 10px 30px color-mix(in srgb, #000 16%, transparent)' }}>
            ...
            <div style={{ marginTop: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <div className="search-route-actions">
                <span className="search-route-chip">{typeFilter === 'series' ? (search.trim() ? `/series/${slugifyPosterName(search)}` : '/series') : searchRoutePath(search)}</span>
                {typeFilter !== 'series' && <button className="fpill" type="button" onClick={() => openSeriesMode(search)}>Series only</button>}
                {typeFilter === 'series' && <button className="fpill" type="button" onClick={() => openSearchMode(search)}>All titles</button>}
