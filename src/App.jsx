  ...
  <div className="settings-redesign">
    <section className="settings-hero-card">
      <div>
        ...
        <h2>Personalize your MCU experience</h2>
        <p>Modern controls, cleaner layout, and secure backup tools in one place.</p>
      </div>
      <div className="settings-hero-badges">
        <span>{darkMode ? 'Dark mode' : 'Light mode'}</span>
        <span>{performanceMode ? 'Performance On' : 'Full Effects'}</span>
      </div>
    </section>

    <section className="settings-card">
      <div className="settings-card-head"><h3>Profile</h3></div>
      <input value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} placeholder="User name" className="settings-input" />
      <div className="settings-avatar-grid">{uploadedAvatars.map((src, idx) => (<button key={idx} onClick={() => setProfile(p => ({ ...p, pfp: src }))} title={`Avatar ${idx + 1}`} className="settings-avatar-btn" data-active={profile.pfp===src}><img src={src} alt={`Avatar ${idx + 1}`} /></button>))}
        <label title="Upload custom avatar" className="settings-avatar-upload"><div><Upload size={13} /><span>Custom +</span></div><input type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = () => { const img = String(r.result || ''); setAvatarCropSrc(img); }; r.readAsDataURL(f); }} style={{ display: 'none' }} /></label>
      </div>
    ...
    <section className="settings-card">
      <div className="settings-card-head"><h3>Google Drive Backup Guide</h3></div>
      <ol className="settings-guide-list">
        <li>Tap <b>Sign in with Google</b> to connect your account.</li>
        <li>After login, tap <b>Save to Account</b> to upload this device backup.</li>
        <li>On another phone/browser, sign in to the same Google account.</li>
        <li>Tap <b>Load from Account</b> to restore your saved progress.</li>
      </ol>
      {!googleAccount?.sub ? (<button className="fpill" onClick={openGoogleLogin} style={{ justifyContent: 'center' }}><UserCircle size={14} /> Sign in with Google</button>) : (<><div style={{ fontSize: 12, color: T.text }}><b>{googleAccount.name || 'Google User'}</b> · {googleAccount.email}</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}><button className="fpill" onClick={() => saveDataForGoogleAccount()}><Upload size={14} />Save to Account</button><button className="fpill" onClick={() => loadDataForGoogleAccount()}><Download size={14} />Load from Account</button></div><button className="fpill" onClick={() => { setGoogleAccount(null); removeStorageValue('mcu-google-account-v1'); setGoogleSyncMsg('Signed out locally.'); }} style={{ justifyContent: 'center' }}><XCircle size={14} /> Sign out</button></>)}
      {!!googleSyncMsg && <div style={{ fontSize: 11, color: T.textMuted }}>{googleSyncMsg}</div>}
    </section>

    <section className="settings-card">
      <div className="settings-card-head"><h3>Preferences</h3></div>
      <div className="settings-toggle-list">
        <label><span><Star size={14}/>Marvel Easter Egg Language</span><button className='fpill settings-toggle-pill' type='button' aria-pressed={marvelLangMode} onClick={() => setMarvelLangMode(v => !v)}>{marvelLangMode ? 'On' : 'Off'}</button></label>
        <label><span><Moon size={14}/>{tMarvel('Dark Theme')}</span><button className='fpill settings-toggle-pill' type='button' aria-pressed={darkMode} onClick={() => setDarkMode(d => !d)}>{darkMode ? 'On' : 'Off'}</button></label>
        <label><span><EyeOff size={14}/>{tMarvel('Spoiler Safe')}</span><button className='fpill settings-toggle-pill' type='button' aria-pressed={spoilerSafeMode} onClick={() => setSpoilerSafeMode(v => !v)}>{spoilerSafeMode ? 'On' : 'Off'}</button></label>
        <label><span><Zap size={14}/>{tMarvel('Performance Mode')}</span><button className='fpill settings-toggle-pill' type='button' aria-pressed={performanceMode} onClick={() => setPerformanceMode(v => !v)}>{performanceMode ? 'On' : 'Off'}</button></label>
      </div>
    </section>

    <section className="settings-card"><div className="settings-card-head"><h3>Backup & Restore</h3></div><button className="fpill" onClick={exportProgress}><Download size={14}/>Export Backup JSON</button><label className="fpill import-backup-pill" style={{ cursor: 'pointer' }}><Upload size={14}/>Import Backup JSON<input type="file" accept="application/json" onChange={(e) => importProgress(e.target.files?.[0])} style={{ display: 'none' }} /></label></section>
  </div>
</SettingsMenu>
