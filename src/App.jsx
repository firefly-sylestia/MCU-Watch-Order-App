...
import { buildSemanticThemeVars, THEME_STATUS_TOKENS, UI_PARITY_TOKENS } from './constants/ui';
...
const TYPE_META = {
  film:   { label: 'Film',   Icon: Film, color: THEME_STATUS_TOKENS.film },
  series: { label: 'Series', Icon: Tv,   color: THEME_STATUS_TOKENS.series },
  short:  { label: 'Short',  Icon: Zap,  color: THEME_STATUS_TOKENS.short },
};

const FALLBACK_TYPE_META = { label: 'Title', Icon: Layers, color: 'var(--theme-text-secondary)' };

const STATUS_META = {
  watched:        { label: 'Completed',      color: THEME_STATUS_TOKENS.watched, Icon: Check,      bg: 'color-mix(in srgb, var(--status-complete) 14%, transparent)'  },
  'plan-to-watch':{ label: 'Watchlist',      color: THEME_STATUS_TOKENS.watchlist, Icon: Bookmark,   bg: 'color-mix(in srgb, var(--status-watchlist) 14%, transparent)' },
  watching:       { label: 'In Progress',    color: THEME_STATUS_TOKENS.watching, Icon: PlayCircle, bg: 'color-mix(in srgb, var(--status-watching) 14%, transparent)' },
  'on-hold':      { label: 'Paused',         color: THEME_STATUS_TOKENS.paused, Icon: PauseCircle,bg: 'color-mix(in srgb, var(--status-paused) 14%, transparent)' },
  dropped:        { label: 'Dropped',        color: THEME_STATUS_TOKENS.dropped, Icon: XCircle,    bg: 'color-mix(in srgb, var(--status-dropped) 14%, transparent)'  },
  ...
  return (
    <>
      <div className="sidebar-control-cluster" style={controlsHidden ? { opacity: 0, pointerEvents: 'none', visibility: 'hidden' } : undefined}>
        <button className="theme-btn sidebar-toggle-btn" onClick={onToggle} aria-label="Toggle sidebar menu" style={{ background: 'var(--control-solid-bg)', color: 'var(--theme-text)', borderColor: 'var(--theme-border)', boxShadow: 'none' }}><Menu size={18} /></button>
        <button className="theme-btn sidebar-toggle-btn settings-toggle-btn" onClick={onOpenSettings} aria-label="Open settings and profile" style={{ background: 'var(--control-solid-bg)', color: 'var(--theme-text)', borderColor: 'var(--theme-border)', boxShadow: 'none' }}><Settings size={18} /></button>
      </div>
      <div className="sidebar-backdrop" data-state={open ? 'open' : 'closed'} onPointerDown={(e) => { e.preventDefault(); onClose?.(); }} />
      <aside ref={ref} data-state={open ? 'open' : 'closed'} aria-hidden={!open} className="sidebar-menu" style={{ '--sidebar-bg': 'var(--theme-overlay-surface)', '--sidebar-border': 'var(--theme-overlay-border)', '--sidebar-transform': open ? 'translateX(0)' : 'translateX(-105%)', '--sidebar-shadow': 'var(--theme-shadow)', '--sidebar-blur': performanceMode ? 'none' : 'blur(8px)' }}>
        {children}
      </aside>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="settings-shell" data-state={open ? 'open' : 'closed'} role="dialog" aria-modal={open ? 'true' : 'false'} aria-hidden={!open} aria-label="Settings and profile" ref={ref}>
        <div className="fade-in settings-menu settings-menu-redesign" data-state={open ? 'open' : 'closed'} style={{ '--settings-bg': 'var(--theme-surface-elevated)', '--settings-blur': performanceMode ? 'none' : 'blur(8px)' }}>
...
export default function MCUViewer() {
  ...
  const T = {
    ...
    scrollTrack: 'color-mix(in srgb, var(--theme-bg) 86%, transparent)',
    scrollThumb: 'color-mix(in srgb, var(--theme-accent) 22%, var(--theme-border))',
    scrollThumbH: 'color-mix(in srgb, var(--theme-accent) 36%, var(--theme-border-strong))',
    hexDot: 'color-mix(in srgb, var(--theme-accent) 5%, transparent)',
    switcherBg: 'var(--control-solid-bg)',
  ...
  const themeAccent = activeThemeVars['--theme-accent'];
  const themeAccentAlt = activeThemeVars['--theme-accent-alt'];
  const cssThemeVars = {
    ...semanticThemeVars,
    ...activeThemeVars,
    '--theme-accent': 'var(--accent-1)',
    '--theme-accent-alt': 'var(--accent-2)',
    '--theme-border': semanticThemeVars['--theme-border'],
    '--theme-border-strong': semanticThemeVars['--theme-border-strong'],
    '--theme-text': semanticThemeVars['--theme-text'],
    '--theme-text-secondary': semanticThemeVars['--theme-text-secondary'],
    '--theme-text-muted': semanticThemeVars['--theme-text-muted'],
    '--theme-text-disabled': semanticThemeVars['--theme-text-disabled'],
    ...
    '--theme-overlay-surface': darkMode
      ? `color-mix(in srgb, ${themeAccent} 10%, var(--theme-surface))`
      : `color-mix(in srgb, ${themeAccentAlt} 7%, var(--theme-surface))`,
    '--theme-overlay-border': darkMode
      ? `color-mix(in srgb, ${themeAccentAlt} 24%, var(--theme-border))`
      : `color-mix(in srgb, ${themeAccent} 20%, var(--theme-border))`,
    '--detail-shell-bg': darkMode
      ? 'linear-gradient(145deg, color-mix(in srgb,var(--theme-bg) 94%, #000), color-mix(in srgb,var(--theme-surface) 88%, #000) 54%, color-mix(in srgb,var(--theme-bg-alt) 88%, #000))'
      : 'linear-gradient(145deg, color-mix(in srgb,var(--theme-surface) 90%, var(--theme-bg)), color-mix(in srgb,var(--theme-bg) 92%, #fff) 56%, color-mix(in srgb,var(--theme-surface) 80%, var(--theme-bg)))',
    '--detail-panel-bg': darkMode
      ? 'color-mix(in srgb,var(--theme-surface) 78%, rgba(8,12,26,0.72))'
      : 'color-mix(in srgb,var(--theme-surface) 82%, var(--theme-bg))',
    '--app-bg-base': 'var(--theme-bg)',
    '--app-bg-vignette': darkMode ? 'rgba(2,6,23,0.36)' : 'rgba(37,99,235,0.08)',
    '--app-bg-noise-opacity': darkMode ? '0.045' : '0.025',
    '--theme-app-bg': darkMode
      ? `radial-gradient(circle at 8% 2%, color-mix(in srgb, ${themeAccent} 30%, transparent), transparent 34%), radial-gradient(circle at 90% 8%, color-mix(in srgb, ${themeAccentAlt} 26%, transparent), transparent 40%), radial-gradient(circle at 50% 120%, color-mix(in srgb, ${themeAccent} 12%, transparent), transparent 52%), var(--theme-app-gradient)`
      : `radial-gradient(circle at 8% 4%, color-mix(in srgb, ${themeAccent} 12%, transparent), transparent 34%), radial-gradient(circle at 88% 14%, color-mix(in srgb, ${themeAccentAlt} 10%, transparent), transparent 40%), var(--theme-app-gradient)`,
    '--comp-overlay-bg': 'var(--theme-overlay-surface)',
    '--comp-dropdown-bg': 'var(--dropdown-bg)',
    '--theme-header-bg': darkMode
      ? `linear-gradient(180deg, color-mix(in srgb, ${themeAccent} 14%, var(--theme-bg-alt)), var(--theme-bg))`
      : `linear-gradient(180deg, color-mix(in srgb, ${themeAccent} 7%, var(--theme-surface-solid)), var(--theme-bg))`,
    '--theme-watched-bg': darkMode
      ? `linear-gradient(100deg, color-mix(in srgb, ${themeAccent} 13%, var(--theme-surface)), color-mix(in srgb, ${themeAccentAlt} 8%, var(--theme-surface)))`
      : `linear-gradient(100deg, color-mix(in srgb, ${themeAccent} 10%, var(--theme-surface-solid)), color-mix(in srgb, ${themeAccentAlt} 6%, var(--theme-surface-solid)))`,
  };
