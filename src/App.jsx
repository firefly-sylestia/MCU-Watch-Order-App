    ...
    <div data-theme={themeMode} style={{ ...cssThemeVars, '--row-gap': densityMode === 'compact' ? '8px' : '12px', '--row-pad': densityMode === 'compact' ? '11px 10px 11px 8px' : '16px 16px 16px 12px', '--row-min-h': densityMode === 'compact' ? '72px' : '86px', '--text-scale': 1, '--ui-scale': textScaleEnabled ? desktopTextScale : 1, width: '100%', minHeight: '100dvh', background: appThemeBg, color: 'var(--theme-text)', fontFamily: 'var(--font-marvel-body)', fontSize: '16px', zoom: 'var(--ui-scale)', transformOrigin: 'top left', left: textScaleEnabled ? '0' : 'auto', display: 'flex', flexDirection: 'column', overflow: 'visible', touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', transition: 'background 260ms var(--ease-out), color 180ms var(--ease-out)' }} className={`theme-switch${performanceMode ? ' performance-mode' : ''}${sidebarOpen || settingsOpen ? ' overlay-open' : ''}`}>
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .meta-chip{display:inline-flex;align-items:center;gap:4px;color:var(--theme-text-muted);background:var(--theme-surface);border:1px solid var(--theme-border);border-radius:var(--radius-sm);padding:2px 6px;letter-spacing:1px;font-family:var(--font-marvel-ui);flex-shrink:0;max-width:100%}
        ...
        .sidebar-toggle-btn{position:fixed;top:calc(env(safe-area-inset-top, 0px) + var(--space-4));left:var(--space-4);z-index:var(--z-sidebar);width:44px;height:44px}.sidebar-backdrop{position:fixed;inset:0;background:rgba(4,8,18,0.62);z-index:calc(var(--z-sidebar) + 1);pointer-events:auto}.sidebar-menu{position:fixed;top:0;left:0;bottom:0;width:min(320px,84vw);padding:86px 14px 20px;background:var(--sidebar-bg);backdrop-filter:var(--sidebar-blur);-webkit-backdrop-filter:var(--sidebar-blur);border-right:1px solid var(--sidebar-border);transform:var(--sidebar-transform);transition:transform 0.22s cubic-bezier(.22,.9,.24,1);z-index:calc(var(--z-sidebar) + 2);overflow-y:auto;box-shadow:var(--sidebar-shadow);border-radius:16px}.settings-menu-anchor{position:fixed;top:calc(env(safe-area-inset-top, 0px) + 16px);right:14px;z-index:940}.settings-menu{position:absolute;top:100%;right:0;z-index:50;margin-top:8px;min-width:320px;width:min(360px,calc(100vw - 28px));max-height:min(80vh,calc(100dvh - 92px));border-radius:var(--radius-md);border:1px solid color-mix(in srgb, var(--theme-accent) 35%, transparent);background:var(--settings-bg);backdrop-filter:var(--settings-blur);-webkit-backdrop-filter:var(--settings-blur);box-shadow:none;padding:10px;display:grid;gap:8px;overflow:auto;color:var(--theme-text);overscroll-behavior:contain}.settings-menu .fpill{min-width:0}.overlay-open .hero-backdrop-image{filter:saturate(1.04) contrast(1.01) brightness(.98);opacity:calc(var(--backdrop-opacity,0.9) * .55);transition-duration:180ms}.overlay-open .hero-backdrop-blend{opacity:.34}.overlay-open .dropdown-pop,.overlay-open .dropdown-pop-up{backdrop-filter:none;-webkit-backdrop-filter:none}.floating-controls{position:fixed;right:var(--space-4);bottom:max(var(--space-4), env(safe-area-inset-bottom));z-index:var(--z-fab);display:flex;flex-direction:column;gap:var(--space-3);align-items:flex-end;pointer-events:none}.floating-controls>*{pointer-events:auto}.floating-mode-switch{display:flex;border-radius:999px;overflow:hidden;border:1px solid ${T.surfaceBorder};background:${darkMode ? 'rgba(10,14,28,0.93)' : 'rgba(255,255,255,0.95)'};box-shadow:none}.bottom-action-dock{display:flex;gap:8px;align-items:center;justify-content:flex-end;flex-wrap:wrap}
        .header-brand{display:grid;gap:var(--space-2);padding:var(--space-3) var(--space-4);border-radius:var(--radius-lg);background:linear-gradient(180deg,rgba(5,8,18,.64),rgba(5,8,18,.34));border:1px solid color-mix(in srgb,var(--theme-accent) 22%, rgba(255,255,255,0.12));box-shadow:var(--elevation-surface-1)}
        .header-brand.compact{padding:4px 10px}
        ...
        <div className="header-inner" style={{ width: '100%', maxWidth: 1240, margin: '0 auto', padding: headerMinimized ? 'calc(env(safe-area-inset-top, 0px) + 14px) 24px 10px' : 'calc(env(safe-area-inset-top, 0px) + 26px) 30px 16px', transition: 'padding 0.2s ease' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 6 }}>
            <div className={`header-brand ${headerMinimized ? 'compact' : ''}`} style={{ fontFamily: 'var(--font-marvel-display)', lineHeight: 0.9, marginBottom: 0, fontWeight: 900 }}>
              <div className="header-title-mcu" style={{ fontSize: 'clamp(42px, 8vw, 62px)', letterSpacing: 'clamp(1px, 0.5vw, 4px)', color: '#f8fafc', display: 'inline-block', padding: '2px 14px', margin: '2px 0 8px', background: 'linear-gradient(180deg, rgba(185,28,28,0.62), rgba(127,29,29,0.46))', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 10 }}>MCU</div>
              <div className="header-title-sub" style={{ fontSize: 'clamp(23px, 3.8vw, 34px)', letterSpacing: 'clamp(2px, 0.7vw, 5px)', color: '#e2e8f0', marginTop: 0 }}>VIEWING ORDER</div>
            </div>
          </div>
        </div>
      </header>
