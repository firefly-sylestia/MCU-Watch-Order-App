...
export default function MCUViewer() {
  ...
  const renderPhaseSelector = () => (
    <div ref={phaseRef} style={{ display: 'flex', alignItems: 'center', gap: 10, overflowX: 'auto', paddingBottom: 4, maxWidth: '100%', width: '100%' }}>
      <button className="fpill phase-chip" onClick={() => { setActivePhase(0); setBrowseMode('phase'); }} style={{ borderRadius: 999, borderColor: activePhase === 0 ? 'var(--theme-accent)' : T.filterBorder, background: activePhase === 0 ? 'color-mix(in srgb, var(--theme-accent) 14%, var(--theme-surface))' : 'var(--chip-bg)', color: activePhase === 0 ? 'var(--theme-accent)' : 'var(--text-secondary)' }}>
        <span style={{ fontSize: 12, letterSpacing: 1.1, whiteSpace: 'nowrap', color: 'inherit' }}>All</span>
        <ChevRight size={12} style={{ opacity: 0.78 }} />
      </button>
