...
export default function MCUViewer() {
  ...
  return (
    <div data-scaffold={Boolean(sectionScaffold)} data-theme={themeMode} style={{ ...cssThemeVars, '--row-gap': densityMode === 'compact' ? '8px' : '12px', '--row-pad': densityMode === 'compact' ? '11px 10px 11px 8px' : '16px 16px 16px 12px', '--row-min-h': densityMode === 'compact' ? '72px' : '86px', '--text-scale': 1, '--ui-scale': effectiveUiScale, minHeight: '100dvh', backgroundColor: 'var(--app-bg-base)', backgroundImage: appTexture !== 'none' ? `${appTexture}, ${appThemeBg}` : appThemeBg, backgroundSize: appTexture !== 'none' ? '6px 6px, auto' : 'auto', color: 'var(--theme-text)', fontFamily: 'var(--font-marvel-body)', fontSize: '16px', zoom: effectiveUiScale, display: 'flex', flexDirection: 'column', overflow: 'visible', touchAction: 'pan-y', WebkitOverflowScrolling: 'touch', transition: 'background 260ms var(--ease-out), color 180ms var(--ease-out)' }} className={`theme-switch${performanceMode ? ' performance-mode' : ''}${overlayActive ? ' overlay-open' : ''}`} data-color-mode={darkMode ? 'dark' : 'light'}>
      ...
      {/* ━━ POSTER CAROUSEL ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
      <section className="wf-slider" aria-label="Featured MCU posters" style={{ maxWidth: 1240, margin: '0 auto', width: 'min(1240px, calc(100% - 24px))', flexShrink: 0 }}>
        {heroPosters.length > 0 && (
          <>
            <button className="wf-slider-nav wf-slider-nav-prev" type="button" aria-label="Previous featured poster" onClick={() => { pauseHeroAutoSlide(2600); setHeroIndex(i => (i - 1 + heroPosters.length) % heroPosters.length); }}>
              <ChevRight size={16} style={{ transform: 'rotate(180deg)' }} />
            </button>
            <div className="wf-slider-track"
              ...
              {visibleHeroPosters.map(({ src, item: heroItem }, idx) => {
                ...
                return (
                  <article key={`hero-rail-${src}`} ref={isActive ? heroActiveCardRef : null} className={`wf-slide ${isActive ? 'is-active' : ''}`} onClick={() => { if (heroItem) openDetail(heroItem); }}>
                    <img className="wf-slide-image" src={src} alt={heroItem?.title || 'Featured MCU poster'} draggable={false} loading={idx < 8 ? 'eager' : 'lazy'} decoding="async" onDragStart={(e) => e.preventDefault()} />
                    <div className="wf-slide-title">{heroItem?.title || 'Featured MCU poster'}</div>
                  </article>
                );
              })}
            </div>
            <button className="wf-slider-nav wf-slider-nav-next" type="button" aria-label="Next featured poster" onClick={() => { pauseHeroAutoSlide(2600); setHeroIndex(i => (i + 1) % heroPosters.length); }}>
              <ChevRight size={16} />
            </button>
          </>
        )}
