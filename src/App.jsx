...
import { UI_PARITY_TOKENS } from './constants/ui';
import { TYPE_COLOR_TOKENS, STATUS_COLOR_TOKENS, composeColorModeCssVars, composeLegacyTheme } from './constants/themePalette';
...
const TYPE_META = {
  film:   { label: 'Film',   Icon: Film, color: TYPE_COLOR_TOKENS.film },
  series: { label: 'Series', Icon: Tv,   color: TYPE_COLOR_TOKENS.series },
  short:  { label: 'Short',  Icon: Zap,  color: TYPE_COLOR_TOKENS.short },
};

const FALLBACK_TYPE_META = { label: 'Title', Icon: Layers, color: 'var(--theme-text-secondary)' };

const STATUS_META = {
  watched:        { label: 'Completed',      color: STATUS_COLOR_TOKENS.watched.color, Icon: Check,      bg: STATUS_COLOR_TOKENS.watched.bg  },
  'plan-to-watch':{ label: 'Watchlist',      color: STATUS_COLOR_TOKENS['plan-to-watch'].color, Icon: Bookmark,   bg: STATUS_COLOR_TOKENS['plan-to-watch'].bg },
  watching:       { label: 'In Progress',    color: STATUS_COLOR_TOKENS.watching.color, Icon: PlayCircle, bg: STATUS_COLOR_TOKENS.watching.bg },
  'on-hold':      { label: 'Paused',         color: STATUS_COLOR_TOKENS['on-hold'].color, Icon: PauseCircle,bg: STATUS_COLOR_TOKENS['on-hold'].bg },
  dropped:        { label: 'Dropped',        color: STATUS_COLOR_TOKENS.dropped.color, Icon: XCircle,    bg: STATUS_COLOR_TOKENS.dropped.bg  },
...
const SidebarMenu = React.memo(React.forwardRef(function SidebarMenu({
  ...
  return (
    <>
      ...
      <aside ref={ref} data-state={open ? 'open' : 'closed'} aria-hidden={!open} className="sidebar-menu" style={{ '--sidebar-bg': 'var(--theme-overlay-surface)', '--sidebar-border': surfaceBorder, '--sidebar-transform': open ? 'translateX(0)' : 'translateX(-105%)', '--sidebar-shadow': 'var(--shadow-elevated)', '--sidebar-blur': performanceMode ? 'none' : 'blur(8px)' }}>
        {children}
      </aside>
    </>
  );
...
export default function MCUViewer() {
  ...
  const colorModeVars = composeColorModeCssVars(darkMode, activeThemeVars);
  const T = composeLegacyTheme(darkMode);
