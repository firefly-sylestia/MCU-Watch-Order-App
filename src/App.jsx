...
export default function MCUViewer() {
  ...
  const openSidebarHubFromSettings = useCallback(() => {
    setSettingsOpen(false);
    setAnalyticsOpen(false);
    setSidebarOpen(true);
  }, []);
  ...
  <div className="settings-redesign" aria-label="Grouped More commands">
    <section className="settings-hero-card settings-command-group settings-command-group--hero">
      <div>
        ...
        <p>Preferences, display options, data backup, and advanced utilities are grouped here. Fast style and theme controls live in the Sidebar Hub.</p>
      </div>
      <div className="settings-hero-actions">
        <button className="fpill" type="button" onClick={openSidebarHubFromSettings} aria-controls="sidebar-hub"><Layers size={14}/>Open Sidebar Hub</button>
        <button className="fpill" type="button" onClick={exportProgress}><Download size={14}/>Export backup</button>
      </div>
    </section>

    <section className="settings-command-grid" aria-label="More command groups">
      <article className="settings-card settings-command-group">
        <p className="settings-eyebrow">App Preferences</p>
        <h3>Profile & comfort</h3>
        <input value={profile.name} onChange={e => setProfile(p => ({ ...p, name: e.target.value }))} placeholder="User name" className="settings-input" aria-label="Profile name" />
        <div className="settings-avatar-grid" aria-label="Profile avatar choices">
          {PROFILE_PICS.map((src, idx) => (
            <button key={idx} type="button" onClick={() => setProfile(p => ({ ...p, pfp: src }))} title={`Avatar ${idx + 1}`} aria-pressed={profile.pfp === src} className="settings-avatar-btn" style={{ background: profile.pfp === src ? 'var(--theme-surface-hover)' : 'transparent' }}>
              <img src={src} alt="" />
            </button>
          ))}
          <label title="Upload custom avatar" className="settings-avatar-upload">
            <Upload size={16}/>
            <input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) setAvatarCropSrc(URL.createObjectURL(file)); }} style={{ display: 'none' }} />
          </label>
        </div>
      ...
      <article className="settings-card settings-command-group">
        <p className="settings-eyebrow">Display & Theme</p>
        <h3>Mode and visual tuning</h3>
        <p className="settings-help">Use the Sidebar Hub for compact style cards, character accents, and universe context. These controls fine-tune display behavior.</p>
        <div className="settings-inline-actions">
          <button className="fpill" type="button" onClick={openSidebarHubFromSettings}><Layers size={14}/>Open Sidebar Hub</button>
          <button className="fpill" type="button" aria-pressed={darkMode} onClick={() => setDarkMode(v => !v)}>{darkMode ? <Moon size={14}/> : <Sun size={14}/>}{darkMode ? 'Dark mode' : 'Light mode'}</button>
        </div>
        ...
        <div className="settings-scale-grid" aria-label="Desktop text scale">
      ...
      <article className="settings-card settings-command-group">
        <p className="settings-eyebrow">Help / About</p>
    ...
    <section className="settings-card settings-command-group settings-command-group--advanced">
      <p className="settings-eyebrow">Advanced</p>
