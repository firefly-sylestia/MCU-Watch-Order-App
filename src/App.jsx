...
const SidebarMenu = React.memo(React.forwardRef(function SidebarMenu({
  ...
  return (
    <>
      ...
      <button className="theme-btn sidebar-toggle-btn" onClick={onToggle} aria-label="Toggle sidebar menu" style={{ background: 'var(--control-solid-bg)', color: 'var(--theme-text)', borderColor: 'var(--theme-border-strong)', boxShadow: 'none' }}><Menu size={18} /></button>
      <button className="theme-btn sidebar-toggle-btn settings-toggle-btn" onClick={onOpenSettings} aria-label="Open settings and profile" style={{ background: 'var(--control-solid-bg)', color: 'var(--theme-text)', borderColor: 'var(--theme-border-strong)', boxShadow: 'none' }}><Settings size={18} /></button>
      </div>
      <div className="sidebar-backdrop" data-state={open ? 'open' : 'closed'} onPointerDown={(e) => { e.preventDefault(); onClose?.(); }} />
      <aside ref={ref} data-state={open ? 'open' : 'closed'} aria-hidden={!open} className="sidebar-menu" style={{ '--sidebar-bg': 'color-mix(in srgb, var(--theme-surface) 88%, transparent)', '--sidebar-border': surfaceBorder, '--sidebar-transform': open ? 'translateX(0)' : 'translateX(-105%)', '--sidebar-shadow': 'var(--fx-shadow-2)', '--sidebar-blur': performanceMode ? 'none' : 'blur(var(--fx-blur))' }}>
        {children}
      </aside>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="settings-shell" data-state={open ? 'open' : 'closed'} role="dialog" aria-modal={open ? 'true' : 'false'} aria-hidden={!open} aria-label="Settings and profile" ref={ref}>
        <div className="fade-in settings-menu settings-menu-redesign" data-state={open ? 'open' : 'closed'} style={{ '--settings-bg': 'color-mix(in srgb, var(--theme-surface) 92%, var(--theme-bg))', '--settings-blur': performanceMode ? 'none' : 'blur(var(--fx-blur))' }}>
...
export default function MCUViewer() {
  ...
  const T = {
    ...
    rowHoverBg: 'var(--interactive-hover-bg)',
    ...
    dropdownShadow: 'var(--fx-shadow-2)',
    text: 'var(--theme-text)',
    textMuted: 'var(--theme-text-muted)',
    textFaint: 'var(--theme-text-disabled)',
    sortHoverBg: 'var(--interactive-hover-bg)',
  ...
  const cssThemeVars = {
    ...
    '--theme-overlay-surface': darkMode
      ? `color-mix(in srgb, ${activeThemeVars['--accent-1']} 14%, rgba(255,255,255,0.06))`
      : `color-mix(in srgb, ${activeThemeVars['--accent-2']} 10%, rgba(15,23,42,0.04))`,
    '--theme-overlay-border': darkMode
      ? `color-mix(in srgb, ${activeThemeVars['--accent-2']} 32%, rgba(255,255,255,0.14))`
      : `color-mix(in srgb, ${activeThemeVars['--accent-1']} 26%, rgba(15,23,42,0.14))`,
    ...
    '--control-solid-bg': 'color-mix(in srgb, var(--theme-surface) 86%, transparent)',
    '--theme-input-bg': 'color-mix(in srgb, var(--theme-surface) 78%, var(--theme-bg))',
    '--theme-scroll-thumb': 'color-mix(in srgb, var(--theme-accent) 34%, var(--theme-border))',
    '--theme-scroll-thumb-hover': 'color-mix(in srgb, var(--theme-accent-alt) 46%, var(--theme-border))',
    '--theme-pattern-dot': 'color-mix(in srgb, var(--theme-accent) 10%, transparent)',
    ...
    '--theme-app-bg': darkMode
      ? `radial-gradient(circle at 8% 2%, color-mix(in srgb, ${activeThemeVars['--accent-1']} 40%, transparent), transparent 34%), radial-gradient(circle at 90% 8%, color-mix(in srgb, ${activeThemeVars['--accent-2']} 36%, transparent), transparent 40%), radial-gradient(circle at 50% 120%, rgba(14,165,233,0.22), transparent 52%), linear-gradient(138deg, #02030a 0%, #09071a 30%, #0f1031 58%, #1a1038 100%)`
      : `radial-gradient(circle at 8% 4%, color-mix(in srgb, ${activeThemeVars['--accent-1']} 15%, #e9e1d5), transparent 34%), radial-gradient(circle at 88% 14%, color-mix(in srgb, ${activeThemeVars['--accent-2']} 13%, #e9e1d5), transparent 40%), linear-gradient(140deg, #e5ddd1 0%, #dfd6c8 44%, #e4dbcf 100%)`,
    ...
    '--theme-header-bg': darkMode
      ? `linear-gradient(180deg, color-mix(in srgb, ${activeThemeVars['--accent-1']} 18%, #0c1022), #06060f)`
      : `linear-gradient(180deg, color-mix(in srgb, ${activeThemeVars['--accent-1']} 9%, #f2ede3), #ece5d9)`,
    '--theme-watched-bg': darkMode
      ? `linear-gradient(100deg, color-mix(in srgb, ${activeThemeVars['--accent-1']} 18%, rgba(12,18,34,0.62)), color-mix(in srgb, ${activeThemeVars['--accent-2']} 10%, rgba(10,20,32,0.54)))`
      : `linear-gradient(100deg, color-mix(in srgb, ${activeThemeVars['--accent-1']} 14%, #ffffff), color-mix(in srgb, ${activeThemeVars['--accent-2']} 8%, #f7f5ef))`,
  };
